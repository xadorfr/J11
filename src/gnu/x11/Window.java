...
public class Window extends Drawable implements GLXDrawable {
	 ...
	 * that member variable <code>display</code> is <code>null</code>. That is,
	 * they are not connected to any X server, and cannot be used for server
	 * interaction (because <code>display == null</code>, resulting in
	 * <code>NullPointerException</code>).
	 * 
	 * <p>
	 * For special operations like setting selection owner to {@link #NONE}, do:
	 * 
	 * <pre>
	 * <code>
	 *   Window.NONE.display = ...;
	 ...
	 * <code>Window.POINTER_WINDOW</code>. Second, it is a better classification
	 * to put predefined resources in their respective classes. Third, it is
	 * cheaper to have predefined resources as <code>static final</code>
	 * objects.
	 * 
	 * <p>
	 ...
	 * (this, 0)</code> will not return static variable {@link #NONE}. Instead
	 * it creates a new <code>Window</code> object which sequential intern calls
	 * will return. As a side effect, <code>Window</code> equality should not be
	 * tested with <code>==</code> operator; it should be tested easily as
	...
	public static class Attributes extends ValueList {
		...
		/**
		 * @param i
		 *            valid: {@link #NOT_USEFUL}(default), {@link #WHEN_MAPPED},
		 *            {@link #ALWAYS}
		 */
	...
	public static class Changes extends ValueList {
		...
		/**
		 * @param i
		 *            valid: {@link #ABOVE}, {@link #BELOW}, {@link #TOP_IF},
		 *            {@link #BOTTOM_IF}, {@link #OPPOSITE}
		 */
	 ...
	 * 
	 * @param mode
	 *            valid: {@link #REPLACE}, {@link #PREPEND}, {@link #APPEND}
	 * 
	 * @param format
	 *            : valid: <code>8</code>, <code>16</code>, <code>32</code>
	 * 
	 * @param data_format
	 *            : valid: <code>8</code>, <code>16</code>, <code>32</code>
	 * 
	...
	public class Property {
		 ...
		 */
		public int value() {
			/* In the Java Virtual Machine, bytes, shorts and ints are all
			 * four bytes long. Hence, when you add two bytes together you are
			 * actually performing 32-bit arithmatic.
			 * The (& 0XFF) ensure to work with the real byte value instead of the 
			 * corresponding 32 bits value which would break the OR operation between bytes
			 */
			int intValue;
			switch (format) {
			case 8:
				intValue = (int) value[0] & 0xff;
				break;
			case 16:
				intValue = (int) ((value[0] & 0xFF) << 8 | ((value[1] & 0xFF)));
				break;
			case 32:
	...
	public void ungrab_key(int keysym, int modifiers) {
		int keycode = keysym == 0 ? 0 : display.input.keysym_to_keycode(keysym);
	...
	public void change_property(Atom property, Atom type, int data) {
		change_property(REPLACE, property, type, 32, new byte[] {
	...
	public static class WMHints extends Data {
		 ...
		 *         {@link #ICON_MASK_HINT_MASK}, {@link #WINDOW_GROUP_HINT_MASK}
	...
	public WMHints wm_hints() {
		...
		// not in the core protocol.
		return null;
		// PropertyReply pi = property (false, Atom.WM_HINTS, Atom.WM_HINTS, 0,
		// 8);
		//
	...
	public String wm_class() {
		Property pi = get_property(false, Atom.WM_CLASS, Atom.STRING, 0,
				MAX_WM_LENGTH);
	...
	public int wm_pid() {
		...
		Property p = get_property(false, wm_pid, gnu.x11.Atom.CARDINAL,
				0, 1);
	
		if(p.string_value().length() == 0) {
			return -1;
		}
	...
	public int wm_state() {
		...
		Property p = get_property(false, wm_state, gnu.x11.Atom.INTEGER,
				0, 2);
		if(p.string_value().length() == 0) {
			return -1;
		}
